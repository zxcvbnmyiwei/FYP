...
function AdminPage() {
          ...
          <tbody>
            {contentArr.map((content) => (
              <tr>
                ...
                <td><input style={{ background: "red", width: "30px" }} onClick={() => handleDelete(content.id, topicChosen.topic)} type='button' value='-' /></td>
                <td><input style={{ background: "red", width: "30px" }} onClick={() => handleEdit(content)} type='button' value='Edit' /></td>
              </tr>
            ))
  ...
  return (
    <div>
      <Button onClick={handleAddTopic}>Click To Add Topic</Button>
      <TopicCreateForm
          open={openTopic}
        ...
        />
      <DynamicDropdown />
      {chosenTopic !== null && <Button onClick={handleAdd}>+</Button>}
